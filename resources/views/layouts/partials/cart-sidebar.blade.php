...
<div class="offcanvas offcanvas-end" tabindex="-1" id="carritoSidebar" aria-labelledby="carritoSidebarLabel">
    ...
    <div class="offcanvas-body d-flex flex-column">
                        ...
                        <div class="list-group-item">
                            <div class="d-flex justify-content-between align-items-start">
                                <div class="flex-grow-1">
                                    <h6 class="mb-1">{{ $item['product']->name }}</h6>     <!-- Nombre del producto accedido desde el modelo relacionado -->
                                    <small class="text-muted">Cant: {{ $item['quantity'] }}</small>
                                </div>

                                <!-- Columna derecha: precio arriba, botón eliminar abajo -->
                                <div class="d-flex flex-column align-items-end gap-1">
                                    <span class="text-warning fw-bold">${{ number_format($item['subtotal'], 2) }}</span>     <!-- Subtotal del item: precio × cantidad. number_format(value, 2) garantiza siempre dos decimales. -->
                                    ...
                                    <form action="{{ route('cart.remove', $item['product']) }}" method="POST" class="cart-remove-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-link p-0 border-0" aria-label="Eliminar {{ $item['product']->name }} del carrito" title="Eliminar">
        ...
        <div class="mt-auto d-flex flex-column gap-2">
            ...
            @if(count($sidebarCartItems) > 0)
                <a href="{{ route('cart.show') }}" class="btn btn-warning w-100">Ver carrito</a>
            @endif
            ...
            <a href="{{ route('products.index') }}" class="btn btn-secondary w-100" data-bs-dismiss="offcanvas">Seguir comprando</a>
        </div>
    </div>
</div>